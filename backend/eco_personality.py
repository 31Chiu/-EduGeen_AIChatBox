...
class EcoPersonality:
    ...
    def __init__(self, config_path=None):
        ...
        self.offset_trees = 0  # Tracks number of trees saved
            ...
            "interaction_behaviors": {
                "text_response": {
                    "footer_template": {
                        ...
                        "zh": "🐻 记住啦：{random_tip} | 虚拟森林已有{offset_trees}棵树",
                        "en": "🐻 Remember: {random_tip} | Virtual forest has {offset_trees} trees"
                    },
                    "random_tips": {
                        "zh": [
                            "晚上关灯省电，猫头鹰睡觉不被打扰 🦉",
    ...
    def _trigger_ecological_alert(self, text):
        ...
        try:
            ...
            print(f"[DEBUG] 生态警报错误: {str(e)}")
            print(f"[DEBUG] 当前配置: {json.dumps(angry_config, indent=2)}")
            return self._add_emoticon(text, 'nature')
    ...
    def _check_quiz_answer(self, user_choice):
        ...
        result = {
            "zh": "答对啦！" if is_correct else "错啦！",
            "en": "Correct! " if is_correct else "Wrong! "
        }[self.current_lang]
        
        response = f"{result}{self.quiz_answers['tip']}"
        self.quiz_answers = {}
        return {"processed": self._add_emoticon(response, 'positive')}
    ...
    def format_response(self, ai_text):
        ...
        if self.interaction_count % self.config['game_settings']['carbon_achievement']['interval'] == 0:
            ...
            achievement = msg.format(count=self.offset_trees)
        ...
        footer = footer_template.format(
            ...
            offset_trees=self.offset_trees,
